...
export default function PdfStatusBar({ indicator, matchInfo, onConfirm, onDispute, onPrev, onNext }) {
  ...
  } else if (indicator.review_status === '未找到') {
    statusLabel = '🔴 未找到';
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3 border-b border-white/5 bg-slate-900/80 shrink-0">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onPrev}
          className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 transition-colors tooltip-wrap"
          title="上一项"
        >
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/30 transition-all flex items-center gap-1"
        >
          ✓ 确认
        </button>
        <button
          onClick={() => onDispute?.(indicator.id)}
          className="px-3 py-1.5 text-xs rounded-lg bg-amber-500/15 hover:bg-amber-500/25 text-amber-400 border border-amber-500/30 transition-all flex items-center gap-1"
        >
          ✗ 不符
        </button>
      </div>
